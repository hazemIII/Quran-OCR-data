...
def plot_img(images):
          ...
          gg.show()
################################################################################
def souraName(image,numberOfLines,souraNameFrameLength):
    delRaws=[]
    filledImage = ndi.binary_fill_holes(image)
    labeledImage,numberOfLabels=label(filledImage,background=0,return_num =True)
    ...
    for comp in labeledImageProperties:
        ...
        left=comp.bbox[1]
        right=comp.bbox[3]
        if(right-left)>image.shape[1]*.8 and (down-up)>.5*(image.shape[0]/numberOfLines):
            count+=1
            delRaws.extend(range(up,down+1))
    image=np.delete(image,delRaws,0)
    numberOfLines=numberOfLines-count
    return numberOfLines,image
################################################################################
################################################################################
################################################################################
...
def find_peaks(image,numberOfLines):
    ...
    for i in range(0,image.shape[0]):
        if (sum(image[i][:])==0) or (sum(image[i][:])>=.4*image.shape[1]):
            deleRaws.append(i)
        else: break
...
def lineSegmantation(inputText,basicsLines):
    ...
    return lines
################################################################################
def main(pageNumberOfLines,pageName,souraNameHeight,count):
    inputImage=np.asarray(mat4py.loadmat(pageName)['text'])
    N,text=souraName(np.asarray(inputImage),pageNumberOfLines,souraNameHeight)
    bases,image= find_peaks(np.asarray(text),N)
    lines=lineSegmantation(np.asarray(image),bases)
    linesName = "lines/lines ("+str(count)+").mat"
    sio.savemat(linesName,mdict={"lines":np.asarray(lines)})
